				...
				elseif rawdataindex+1==2^32
					...
					if strcmpi(index(end).name,'Root')
						...
						if index(end).newob
							...
							if ~kTocRawData %changed from if kTocRawData
...
for i=1:numel(GroupNames)
	...
	if strcmp(cname, 'Root')
		DataStructure.Parameters.Root=ob.(cname);
	end
    ...
    % check if there is more than the data structure, if so, take this as
    % the parameters of the channel
    if isempty(ob.(cname))
        continue;
    end
    substruc=fieldnames(ob.(cname));
    % remove what is associated with the data, since we took care of this
    % before
    if had_data==1
        substruc(find(ismember(substruc, 'data')==1))=[];
        substruc(find(ismember(substruc, 'nsamples')==1))=[];
        substruc(find(ismember(substruc, 'NI_ArrayColumn')==1))=[];  
    end
    
    %whatever is left in substruc is accounted for as parameters, and will
    %be added to DataStruct
    if (numel(substruc)>0 & ~strcmp('Root',cname))
        for j=1:numel(substruc)
           pname=substruc{j}; 
           DataStructure.Parameters.(cname).(pname)=ob.(cname).(pname);
        end
    end
...
function  [FixedText,TruncFieldName,ValidFieldName]=fixcharformatlab(textin)
	...
	textin=strrep(textin,':','');
	textin=strrep(textin,'^','_');
	textin=strrep(textin,'�','Ae');
	textin=strrep(textin,'�','ae');
	textin=strrep(textin,'�','Oe');
	textin=strrep(textin,'�','oe');
	textin=strrep(textin,'�','Ue');
	textin=strrep(textin,'�','ue');
	textin=strrep(textin,'�','ss');
	textin=strrep(textin,'�','e');
	...
	InvalidCharIndices=regexp(textin,'[^A-Za-z_0-9]');		%NOT A thru Z, a thru z, underscore or 0 thru 9
	if ~isempty(InvalidCharIndices)
		...
		if numel(InvalidChar)==1
			fprintf(['\nA valid replacement character has not been defined in the ''fixcharformatlab'' private function\n\t'...
				'(within %s.m) for the invalid character ''%s'' contained within the\n\t''%s'' Group/Channel name.\n\t'...
				'It has been replaced with an ''_''.\n'],mfilename,char(InvalidChar),OrigText)
		else
			for i=1:numel(InvalidChar)
				switch i
					...
					case numel(InvalidChar)
						MyString=sprintf('%s & ''%s''',MyString,InvalidChar{i});
					otherwise
						MyString=sprintf('%s, ''%s''',MyString,InvalidChar{i});
				end
			end
			fprintf(['\nValid replacement characters have not been defined in the ''fixcharformatlab'' private function\n\t'...
				'(within %s.m) for the invalid characters %s contained within the\n\t''%s'' Group/Channel name.\n\t'...
				'They have been replaced with an ''_''.\n'],mfilename,MyString,OrigText)
		end
